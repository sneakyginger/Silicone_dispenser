...
def multi_dispense(amounts, relative_tolerance=0.1, safety_factor=0.95, max_iterations=100):
    ...
    for i in active:
        measured[i] = dispense_and_measure(i + 1, amounts[i])
    ...
    for iterations_used in range(1, max_iterations + 1):
        ...
        for i, shortfall in to_correct:
            ...
            measured[i] += dispense_and_measure(i + 1, correction)
    else:
