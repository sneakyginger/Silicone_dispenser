"""Dispensing progress screen styled like the top bar bucket indicators."""
...
BAR_WARNING_COLOR = theme.CAUTION
...
def _progress_color(dispensed, desired, error):
    if error:
        return BAR_ERROR_COLOR
    if desired <= 0:
        return TRACK_COLOR
    if dispensed > desired * 1.05:
        return BAR_WARNING_COLOR
    if dispensed >= desired - DISPENSE_TOLERANCE_GRAMS:
        return BAR_OK_COLOR
    return BAR_ACTIVE_COLOR


def _draw_component_row(screen, row_index, y, desired, dispensed, error):
    ...
    icon = top_bar._get_bucket_icon(_progress_color(dispensed, desired, error))
    if icon is not None:
        icon_rect = icon.get_rect(midleft=(label_rect.right + ICON_GAP, y))
        screen.blit(icon, icon_rect)

    value = text_font.render(f"{dispensed:.1f} / {desired:.1f} g", True, TEXT_COLOR)
    ...
    if desired > 0:
        if dispensed >= desired - DISPENSE_TOLERANCE_GRAMS:
            ...
            fraction = min(1.0, max(0.0, dispensed / desired))
        fill_w = int(bar_w * fraction)
        if fill_w > 0:
            pygame.draw.rect(
                screen,
                _progress_color(dispensed, desired, error),
                (bar_rect.x, bar_rect.y, fill_w, BAR_HEIGHT),
                border_radius=6,
            )
...
def draw(screen, snapshot):
    ...
    desired = snapshot.get("desired", [0.0, 0.0, 0.0, 0.0])
    dispensed = snapshot.get("dispensed", [0.0, 0.0, 0.0, 0.0])
    start_y = title_rect.bottom + 52
    row_height = BAR_HEIGHT + ROW_GAP
    active_components = [i for i, amount in enumerate(desired) if amount > 0]
    for row, i in enumerate(active_components):
        _draw_component_row(screen, i, start_y + row * row_height, desired[i], dispensed[i], error)

    if error:
        text_font = _get_text_font()
        error_surface = text_font.render(error[:80], True, BAR_ERROR_COLOR)
        error_rect = error_surface.get_rect(midtop=(width // 2, start_y + len(active_components) * row_height + 18))
